...
(function ($) {
  ...
  $.widget('ui.weekCalendar', (function () {
    ...
    ,
       ...
       * resized.
       */
      _isDraggingOrResizing: function (calEvent, $calEvent, $event, $target) {
...
//              calendarHeaderHtml+=    "</div>";
        ...
        $weekDay.mousedown(function (event) {
          ...
          if ($target.hasClass('wc-day-column-inner')) {
            ...
            if (!options.preventDragOnEventCreation) {
              $target.bind('dragstart.mousemove', function (event) {
                $newEvent.show();
    ...
    ,
      ...
      _renderEvent: function (calEvent, $weekDay) {
        ...
        eventHtml += '<div class=\"wc-title ui-corner-top\"></div>';
        eventHtml += '<div class=\"wc-time\"></div></div>';
    ...
    ,
      ...
      _addDraggableToCalEvent: function (calEvent, $calEvent) {
        var self        = this;
        var options     = this.options;
        var eventHeight = $calEvent.height();
        $calEvent.draggable({
          handle: '.wc-time',
          ...
          drag: function (event, ui) {
            var startTime = options.date;
            var newTime   = startTime.clone();
            newTime.addHours(options.businessHours.start);
            var newStart    = new Date(newTime.getTime() + Math.max(1,
              Math.round(ui.position.top / options.timeslotHeight)) *
              options.millisPerTimeslot);
            var newEnd      = new Date(newTime.getTime() + Math.max(1,
              Math.round((ui.position.top + eventHeight) / options.timeslotHeight))
              * options.millisPerTimeslot);
    ...
    ,
      ...
      _addResizableToCalEvent: function (calEvent, $calEvent, $weekDay) {
        var self      = this;
        var options   = this.options;
        var calHeight = options.timeslotHeight * options.timeslotsPerDay;
        $calEvent.resizable({
          ...
          resize: function (event, ui) {
            ...
            var newEnd      = new Date($calEvent.data("calEvent").start.getTime() +
              Math.max(1, Math.round(ui.size.height / options.timeslotHeight)) *
              options.millisPerTimeslot);
            var newCalEvent = $.extend(true, {}, calEvent, {
            ...
            self._refreshEventDetails(newCalEvent, $calEvent);
            var maxHeight = calHeight - ui.originalPosition.top;
          ...
          stop: function (event, ui) {
            ...
            self._adjustForEventCollisions($weekDay, $calEvent, newCalEvent,
              calEvent);
            self._refreshEventDetails(newCalEvent, $calEvent);
            self._positionEvent($weekDay, $calEvent);
            self._adjustOverlappingEvents($weekDay);
  ...
  $.extend(FreeBusyManager.prototype, FreeBusyProto, {
    ...
    insertFreeBusy: function (freeBusy) {
      ...
      var pushNewFreeBusy                  = function (_f) {
        if (_f.isValid()) newFreeBusys.push(_f);
      };
      ...
      //   var str = [];
      //   $.each(tmpFB, function (i) {
      //     str.push(i + ": " + this.getStart().getHours() + ' > ' + this.getEnd().getHours() + ' ' + (this.getOption('free') ? 'free' : 'busy'))
      //   });
      //   console.log(str.join('\n'));
      //
      //   console.log('insert');
      //   var str = [];
      //   $.each(newFreeBusys, function (i) {
      //     str.push(this.getStart().getHours() + ' > ' + this.getEnd().getHours())
      //   });
      //   console.log(str.join(', '));
      //
      //   console.log('results');
      //   var str = [];
      //   $.each(this.freeBusys, function (i) {
      //     str.push(i + ": " + this.getStart().getHours() + ' > ' + this.getEnd().getHours() + ' ' + (this.getOption('free') ? 'free' : 'busy'))
      //   });
      //   console.log(str.join('\n'));
      // }
